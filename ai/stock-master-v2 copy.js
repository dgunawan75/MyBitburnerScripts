...
function rebalancePortfolio(ns, portfolio, config) {
    ...
    for (let [sym, pos] of portfolio.positions) {
        ...
        // Jual short jika forecast naik di atas shortThreshold atau di atas 0.5
        if (pos.shortShares > 0) {
            if (forecast > shortThreshold || forecast > 0.5) {
                let sold = ns.stock.sellShort(sym, pos.shortShares);
                if (sold > 0) {
                    // Untuk short, "jual" berarti menutup posisi dengan membeli kembali
                    // Di Bitburner, sellShort menjual (menutup) posisi short. Revenue = harga jual awal - harga beli kembali? 
                    // Mari gunakan logika: avgShortPrice adalah harga jual awal, sekarang beli kembali di harga ask.
                    let buyPrice = ns.stock.getAskPrice(sym);
                    let revenue = pos.avgShortPrice * pos.shortShares - buyPrice * pos.shortShares - config.COMMISSION;
                    ns.print(`🔺 Tutup SHORT ${sym}: ${pos.shortShares} lot, Profit: $${ns.formatNumber(revenue)}`);
                    freeCash += revenue; // revenue sudah termasuk hasil bersih
                }
            }
        }
    }
    ...
    for (let p of allProspects) {
        ...
        } else if (p.type === 'short' && availableShort > 0) {
            // Cek terlebih dahulu apakah player sudah punya kemampuan Short (BitNode-8)
            // Di Bitburner, jika fitur belum dibuka, fungsinya benar-benar undefined (tidak ada)
            if (typeof ns.stock.shortStock !== "function") {
                p.rating = 0; // Matikan prioritas short untuk putaran ini
                continue;
            }

            let bidPrice = ns.stock.getBidPrice(p.sym);
            let maxSharesCanShort = Math.floor((shareOfCash - config.COMMISSION) / bidPrice);
            let sharesToShort = Math.min(maxSharesCanShort, availableShort);
            if (sharesToShort > 0) {
                let cost = sharesToShort * bidPrice + config.COMMISSION;
                if (cost <= freeCash && cost > config.MIN_TRADE_VALUE) {
                    try {
                        let sold = ns.stock.shortStock(p.sym, sharesToShort);
                        if (sold > 0) {
                            ns.print(`🔴 Short ${p.sym}: ${sharesToShort} lot, harga $${ns.formatNumber(bidPrice)}, diblokir $${ns.formatNumber(cost)}`);
                        }
                    } catch (err) {
                        ns.print(`⚠️ GAGAL SHORTING. (${err})`);
                        p.rating = 0;
                    }
                }
            }
...
function checkStopLossTakeProfit(ns, portfolio, config) {
    for (let [sym, pos] of portfolio.positions) {
        ...
        if (pos.shortShares > 0) {
            // Cek terlebih dahulu apakah player sudah punya kemampuan Short (BitNode-8)
            if (typeof ns.stock.sellShort !== "function") continue;

            let sellPrice = pos.avgShortPrice; // harga jual awal
            let change = (sellPrice - currentAsk) / sellPrice; // positif jika harga turun (untung)
            // Take profit (harga turun)
            if (change >= config.TAKE_PROFIT) {
                let covered = ns.stock.sellShort(sym, pos.shortShares);
                if (covered > 0) {
                    let profit = (sellPrice - currentAsk) * pos.shortShares - config.COMMISSION;
                    ns.print(`💰 Take Profit SHORT ${sym}: untung $${ns.formatNumber(profit)}`);
                }
            }
            // Stop loss (harga naik)
                ...
                let covered = ns.stock.sellShort(sym, pos.shortShares);
                if (covered > 0) {
                    let loss = (currentAsk - sellPrice) * pos.shortShares + config.COMMISSION;
                    ns.print(`🛑 Stop Loss SHORT ${sym}: rugi $${ns.formatNumber(loss)}`);
                }
            }
        }
    }
